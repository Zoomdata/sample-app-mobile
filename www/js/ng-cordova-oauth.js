...
(function() {
  ...
  angular.module('oauth.generic', ['oauth.utils'])
    .factory('$ngCordovaGeneric', generic);

  function generic($q, $http, $cordovaOauthUtility) {
    return { signin: oauthGeneric };

    /*
     * Sign into the ZD generic service
     *
     * @param    string clientId
     * @param    appScope array
     * @param    object options
     * @return   promise
     */
    function oauthGeneric(clientId, authUri, appScope, options) {
      ...
      if(window.cordova) {
        ...
        if($cordovaOauthUtility.isInAppBrowserInstalled()) {
          ...
          if(options !== undefined) {
            if(options.hasOwnProperty("redirect_uri")) {
              redirect_uri = options.redirect_uri;
              console.log('Redirect is ' + redirect_uri);
            }
          }
          console.log('window ');
          console.log(window === undefined ? 'undefined' : ' exists');
          console.log('window.cordova');
          console.log(window.cordova === undefined ? 'undefined' : ' exists');
          console.log('window.cordova.InAppBrowser');
          console.log(window.cordova.InAppBrowser === undefined ? 'undefined' : ' exists');

          var endpoint = authUri + '?client_id=' + clientId + '&redirect_uri=' + redirect_uri + '&response_type=token&scope=' + appScope.join(" ");
          var browserRef = window.cordova.InAppBrowser.open(endpoint, '_blank', 'location=no,clearsessioncache=yes,clearcache=yes');
